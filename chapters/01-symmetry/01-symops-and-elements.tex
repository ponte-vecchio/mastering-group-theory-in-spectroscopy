...
1. The \keyword{Identity Operation}, (\symop{E}) does nothing and has no \keyword{symmetry element} but it is a required member of each symmetry group. Thus, operation with \symop{E} will change neither the positions of the atoms nor the phase of the \porb{z}s.
...
\footnotetext{Since the clockwise $\symop[4]{C}^3$ operation is equivalent to a counter-clockwise \symop[4]{C} rotation, the \symop[4]{C} and $\symop[4]{C}^3$ operations are also referred to as the $\symop[4]{C}^+$ and $\symop[4]{C}^-$ operations respectively.}
